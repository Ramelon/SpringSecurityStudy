...
import com.ramelon.security.security.RamelonUserDetails;
...
public class UserDetailsServiceImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        if (ObjectUtil.isNull(user)) {
            ...
            return new RamelonUserDetails(username, user.getPassword(), user.getPhone(), authorityList,
                    true, true, true, true);
        }
    }
    ...
    public UserDetails loadUserByPhone(String phone) throws UsernameNotFoundException {
        ...
        if (ObjectUtil.isNull(user)) {
            ...
            List<GrantedAuthority> authorityList = AuthorityUtils.commaSeparatedStringToAuthorityList("admin");
            // 3. 返回UserDetails类型用户 // 账号状态这里都直接设置为启用，实际业务可以存在数据库中
            return new RamelonUserDetails(user.getUserName(), null, user.getPhone(), authorityList,
                    true, true, true, true);
        }
    }
}